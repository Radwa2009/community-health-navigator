...
├── index.html            # Complete app: all six views, styles, demo data, and logic in one file
├── README.md             # This file
└── PORTFOLIO_NOTES.md    # Short portfolio description + interview talking points
```

The app is intentionally shipped as a single self-contained `index.html` (embedded CSS and JavaScript) so it deploys anywhere with zero configuration. Inside the file, the code is organized in clearly commented sections: styles, demo data + storage adapter, then application logic.

**Firebase note:** all persistence goes through one `Store` object (in the data section of `index.html`). To move to Firestore later, replace the bodies of `Store.load` and `Store.save` with Firestore reads/writes — nothing else needs to change.
